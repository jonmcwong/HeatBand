...
void setup() {
  ...
  HS.write('a', stuff);
  digitalWrite(heaterSwitchPin1, HIGH);
  digitalWrite(heaterSwitchPin2, HIGH);
  digitalWrite(heaterSwitchPin3, HIGH);
...
void loop () {
  ...
  if (HS.read()) {
    ...
    switch (HS.get_opcode()) {
      case 'l':     //for changing capacity
        capacityConsumed = *HS.get_operand(0);
  ...
  if (!pause) {
    static bool check = true;
          unsigned long vDataArray[3] = {v1, v2, v3}; 
            HS.write('a', vDataArray);
            Serial.println();
    if (check || ((unsigned int)(unsigned int)millis()- tStamp) >= 10000) {
